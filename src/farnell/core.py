    ...
    def parse(self):
        ...
        if self.downloadOK:
            ...
            if pagetest != []:
                ...
                stock = soup.find('div', attrs={'id':'priceWrap'})
                
                stock_ = stock.find('p')
                #print stock_
                if len(stock_.contents) < 2:
                    stock = stock.find('div', attrs={'class':'pdpToolTip'})
                    stock = stock.find('a').contents[0].encode('utf-8').strip()
                else:
                    stock = stock_.contents[2].encode('utf-8')
